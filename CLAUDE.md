...
- **Kanban boards are Markdown.** `##` headings are columns, `- [ ] ` / `- [x] ` items are cards. Parser/serializer is `packages/backend/src/lib/kanban.ts`. Saving a board always sets `kanban: true` in frontmatter.
